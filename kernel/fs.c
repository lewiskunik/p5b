...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;
  //cprintf("off is %d, n (#bytes to read) is: %d\n", off, n);
  if(ip->type == T_DEV){
  ...
  if(ip->type == T_SMALLFILE){
  	...
  	for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
   	...
   	bp = bread(ip->dev, sector_number);
    	m = min(n - tot, BSIZE - off%BSIZE);
    	memmove(dst, ip->addrs + off%BSIZE, m);
    	brelse(bp);
  	}
  }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if(ip->type == T_FILE || ip->type == T_DIR){
  	...
  	for(tot=0; tot<n; tot+=m, off+=m, src+=m){
  	  uint sector_number = bmap(ip, off/BSIZE); //buffer cache mapping
  	  if(sector_number == 0){ //failed to find block
  ...
  if(ip->type == T_SMALLFILE){
	...
	if(off > ip->size || off + n < off) //if out of bounds, return -1
  	...
  	for(tot=0; tot<n; tot+=m, off+=m, src+=m){
  	  uint sector_number = bmap(ip, off/BSIZE); //should pretty much be zero for all small files
  	  //if(sector_number == 0){ //failed to find block
  	    //n = tot; //return number of bytes written so far
  	    //break;
  	  //}
	  ...
	  //n-tot is the remainder of bits left over from the write (only nonzero if couldn't write all bytes)
  	  m = min(n - tot, BSIZE - off%BSIZE);
  	  memmove(ip->addrs + off%BSIZE, src, m); //memory is written here
  	  bwrite(bp);
  	  brelse(bp);
  	}
  }
  ...
  if(n > 0 && off > ip->size){
    ip->size = off;
    iupdate(ip); //critical! if inode has changed. do this.
  }
  return n;
}
